...
function credit(sumCredit) {
    let overPayment;
    return overPayment = sumCredit * 0.17 * 5;
}
alert(credit(100));
...
+  Попробуйте включить ввод данных в вашу программу, например с помощью функции prompt(..). Вы можете, например, запросить у пользователя баланс банковского счета. Развлекайтесь и будьте изобретательны!*/
    ...
    const TAX = 0.2;
    const phoneCost = prompt('Введите стоимость телефона:');
    const accessoryCost = prompt('Введите стоимость аксессуара:');
    let sumCost = +phoneCost + +accessoryCost;
    alert('Итого за телефон и аксессуар: ' + sumCost);
    function taxOverPay() {
        return sumCost * TAX * 12;
    }
    alert('Сумма чека с учетом налога составляет: ' + taxOverPay());
    let balance = prompt('Введите баланс: ')
if (taxOverPay() < balance) {
    alert('У тебя осталось денег: ' + (balance - taxOverPay()));
} else {
